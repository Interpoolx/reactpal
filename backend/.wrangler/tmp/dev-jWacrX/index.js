...
// .wrangler/tmp/bundle-DviUz7/strip-cf-connecting-ip-header.js
  ...
  "../packages/core-registry/src/ModuleRegistry.ts"() {
    ...
    ModuleRegistryClass = class {
      ...
      isModuleAvailable(module, context) {
        ...
        if (!context.isSuperAdmin && context.enabledModules) {
          if (!availability.defaultEnabled && !context.enabledModules.includes(module.id)) {
            return false;
          }
        }
        return true;
...
function registerUsersRoutes(app2) {
    ...
    try {
      let query = `
        SELECT u.id, u.username, u.email, u.first_name, u.last_name, u.role, u.status,
               u.email_verified, u.last_login_at, u.created_at, u.tenant_id
        FROM users u
  ...
  roles.get("/", async (c) => {
    ...
    try {
      ...
      return c.json({ error: "Failed to fetch roles", message: error.message }, 500);
    }
  });
  roles.post("/", async (c) => {
    const db = c.env.DB;
...
function registerTenantsRoutes(app2) {
  ...
  tenants.get("/", async (c) => {
    ...
    try {
      let query = `
        SELECT t.id, t.name, t.slug, td.domain, t.status, t.plan_name, t.billing_status,
               t.current_users, t.max_users, t.storage_used, t.max_storage,
               t.mrr, t.trial_ends_at, t.last_activity_at, t.created_at
        FROM tenants t
      ...
      if (search) {
        query += " AND (name LIKE ? OR slug LIKE ? OR domain LIKE ? OR owner_email LIKE ?)";
        const searchTerm = `%${search}%`;
        params.push(searchTerm, searchTerm, searchTerm, searchTerm);
      }
      query += " ORDER BY created_at DESC";
...
// .wrangler/tmp/bundle-DviUz7/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-DviUz7/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
function registerModulesRoutes(app2) {
  ...
  const getModules = /* @__PURE__ */ __name(() => moduleRegistry.getAll(), "getModules");
  modules.get("/", async (c) => {
    const db = c.env.DB;
    try {
      const stats = await db.prepare(`
  ...
  modules.post("/:id/enable", async (c) => {
    ...
    const { enabledBy } = await c.req.json();
    try {
      const tenants = await db.prepare("SELECT id FROM tenants WHERE deleted_at IS NULL").all();
