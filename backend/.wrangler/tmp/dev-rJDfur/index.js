...
// .wrangler/tmp/bundle-mRLSfE/strip-cf-connecting-ip-header.js
...
function registerTenantsRoutes(app2) {
  ...
  tenants.post("/", async (c) => {
    ...
    try {
      await db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `).bind(id, name, slug, config, status, now, now).run();
      if (domain) {
        await db.prepare(`
  ...
  tenants.patch("/:id", async (c) => {
    ...
    const id = c.req.param("id");
    const body = await c.req.json();
    const directFields = ["name", "slug", "status"];
    const updates = [];
    const params = [];
    ...
    const hasDomainUpdate = domain !== void 0;
    const configFields = [
      "ownerEmail",
      "billingEmail",
      "planName",
    ...
    const hasConfigUpdates = configFields.some((f) => body[f] !== void 0);
    if (updates.length === 0 && !hasDomainUpdate && !hasConfigUpdates) {
      return c.json({ error: "No fields to update" }, 400);
    }
    try {
      if (hasConfigUpdates) {
        const existing = await db.prepare("SELECT config FROM tenants WHERE id = ?").bind(id).first();
        const existingConfig = existing?.config ? JSON.parse(existing.config) : {};
        for (const field of configFields) {
          if (body[field] !== void 0) {
            existingConfig[field] = body[field];
          }
        }
        updates.push("config = ?");
        params.push(JSON.stringify(existingConfig));
      }
      if (updates.length > 0) {
        updates.push("updated_at = ?");
        params.push(Math.floor(Date.now() / 1e3));
      ...
      if (hasDomainUpdate) {
        ...
        if (domain) {
          await db.prepare(`
                        INSERT INTO tenant_domains (id, tenant_id, domain, is_primary, created_at)
                        VALUES (?, ?, ?, 1, ?)
                    `).bind(crypto.randomUUID(), id, domain, Math.floor(Date.now() / 1e3)).run();
          if (kv) {
            const tenant = await db.prepare("SELECT name, slug FROM tenants WHERE id = ?").bind(id).first();
            if (tenant) {
              const manifest = await kv.get("tenant_manifest", "json") || {};
              manifest[domain] = { id, slug: tenant.slug, name: tenant.name };
              await kv.put("tenant_manifest", JSON.stringify(manifest));
            }
          }
        }
  ...
  tenants.post("/:id/suspend", async (c) => {
    ...
    try {
      ...
      config.suspendedAt = now;
      config.suspendedReason = reason || "Suspended by administrator";
      await db.prepare(`
                UPDATE tenants SET status = 'suspended', config = ?, updated_at = ?
                WHERE id = ?
            `).bind(JSON.stringify(config), now, id).run();
      return c.json({ success: true });
    } catch (error) {
  ...
  tenants.post("/:id/reactivate", async (c) => {
    ...
    try {
      ...
      delete config.suspendedReason;
      await db.prepare(`
                UPDATE tenants SET status = 'active', config = ?, updated_at = ?
                WHERE id = ?
            `).bind(JSON.stringify(config), now, id).run();
      return c.json({ success: true });
    } catch (error) {
  ...
  tenants.delete("/:id", async (c) => {
    ...
    try {
      ...
      if (permanent) {
        ...
        await db.prepare(`
                    UPDATE tenants SET status = 'archived', config = ?, updated_at = ?
                    WHERE id = ?
                `).bind(JSON.stringify(config), now, id).run();
      }
...
// .wrangler/tmp/bundle-mRLSfE/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-mRLSfE/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
