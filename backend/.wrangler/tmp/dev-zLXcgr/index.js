...
// .wrangler/tmp/bundle-2Vv89u/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-2Vv89u/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-2Vv89u/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
schema.get("/:table", async (c) => {
  ...
  const allowedTables = ["users", "tenants", "invitations", "roles", "permissions", "settings", "audit_logs", "tenant_domains"];
...
var mockBackups = [
  { id: "bk_1", timestamp: new Date(Date.now() - 864e5).toISOString(), size: "4.2 MB", duration: "12s", status: "success" },
  { id: "bk_2", timestamp: new Date(Date.now() - 1728e5).toISOString(), size: "4.1 MB", duration: "11s", status: "success" }
];
database.get("/status", async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      // Added total records
      lastBackup: mockBackups.length > 0 ? "Recent (managed by D1)" : "None",
      databaseId: c.env.DB_ID || "worker-db",
      // Mock backups for UI demonstration
      backups: mockBackups
    });
  } catch (error3) {
  ...
  try {
    ...
    const tables = await Promise.all(tableNames.map(async (name) => {
      ...
      return {
        ...
        columnCount: colInfoResult.results?.length || 0,
        columns: colInfoResult.results
        // Return full column details
      };
    }));
    return c.json({
      data: tables,
  ...
  try {
    ...
    for (const [key, value] of Object.entries(schema_exports)) {
      if (isDrizzleTable(value)) {
        ...
        const remoteColumns = (remoteColumnsResult.results || []).map((col) => ({
          name: col.name,
          type: col.type,
          notnull: col.notnull === 1,
          pk: col.pk === 1,
...
database.get("/migrations", async (c) => {
  ...
  try {
    ...
    try {
      const appliedResult = await db.prepare("SELECT * FROM __drizzle_migrations").all();
      (appliedResult.results || []).forEach((m) => {
        appliedMap.set(String(m.id), {
          id: m.id,
          hash: m.hash,
      ...
      console.warn("Could not read __drizzle_migrations:", e);
    }
    console.log("[Migrations] process.cwd():", process.cwd());
    const possiblePaths = [
    ...
    let migrationFiles = [];
    let usedPath = "None found";
    try {
      for (const p of possiblePaths) {
        console.log("[Migrations] Checking path:", p);
        if (existsSync(p)) {
          migrationFiles = readdirSync(p).filter((f) => f.endsWith(".sql"));
          usedPath = p;
          console.log(`[Migrations] Found ${migrationFiles.length} files at ${p}`);
          break;
        }
      }
    } catch (fsErr) {
      console.warn("[Migrations] FS Access failed:", fsErr.message);
    }
    const allMigrations = migrationFiles.map((filename) => {
      let status = "pending";
      let appliedAt = null;
      let hash = null;
      for (const [id, data] of appliedMap.entries()) {
      ...
      return {
        id: filename,
        // Use filename as unique ID for UI
        name: filename,
        status,
        appliedAt,
        hash
    ...
    allMigrations.sort((a, b) => b.name.localeCompare(a.name));
...
database.post("/migrations/run", async (c) => {
  ...
  try {
    ...
    await db.prepare("INSERT INTO __drizzle_migrations (id, hash, created_at) VALUES (?, ?, ?)").bind(migration.replaceAll(".sql", "").replace("migration_", ""), "manual_apply_" + timestamp, timestamp).run();
    await logAudit(db, "MIGRATION_APPLY", `Marked migration ${migration} as applied locally.`);
    return c.json({
...
database.post("/schema/sync", async (c) => {
  ...
  try {
    ...
    await logAudit(db, "SCHEMA_SYNC", `Synced table ${table3}: Executed ${statements.length} statements.`);
    return c.json({
...
async function logAudit(db, action, details) {
  try {
    await db.prepare(`INSERT INTO audit_logs (id, action, details, created_at) VALUES (?, ?, ?, ?)`).bind(crypto.randomUUID(), action, details, (/* @__PURE__ */ new Date()).toISOString()).run();
  } catch (e) {
    console.warn("Failed to audit log", e);
  }
}
...
database.get("/audit-logs", async (c) => {
  ...
  try {
    ...
    const logs = (result.results || []).map((log3) => ({
      ...
      createdAt: log3.created_at,
      // Map simplified types for UI
      type: log3.action.includes("FAIL") ? "error" : "success"
    }));
    return c.json(logs);
  } catch (e) {
    return c.json([]);
...
database.post("/migrations/rollback", async (c) => {
  const db = c.env.DB;
  try {
    const latest = await db.prepare("SELECT * FROM __drizzle_migrations ORDER BY created_at DESC LIMIT 1").first();
    if (!latest) {
      return c.json({ success: false, message: "No applied migrations to rollback." }, 400);
    }
    await db.prepare("DELETE FROM __drizzle_migrations WHERE id = ?").bind(latest.id).run();
    const migrationName = `migration_${latest.id}`;
    await logAudit(db, "MIGRATION_ROLLBACK", `Rolled back migration ${latest.id}`);
    return c.json({ success: true, message: `Rolled back ${migrationName}. It is now pending.` });
  } catch (e) {
    return c.json({ success: false, message: "Rollback failed: " + e.message }, 500);
  }
});
database.post("/backups", async (c) => {
  ...
  if (mockBackups.length > 10)
    mockBackups = mockBackups.slice(0, 10);
  return c.json({
    success: true,
    message: "Backup triggered successfully (D1 Automated Snapshot)",
    timestamp,
